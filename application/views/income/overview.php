...
<div class="row">
                    ...
                    ?>
						...
						<div class="btn-group">
							...
							<ul class="dropdown-menu">
								<li><a href="#"
									   onclick="load_income_custom('<?php echo date('Y-m'); ?>', '<?php echo date('Y-m'); ?>', 'current_m')">
										Current Month </a></li>
								<li><a href="#"
									   onclick="load_income_custom('<?php echo $dt->format('Y-m'); ?>', '<?php echo $dt->format('Y-m'); ?>','previous_m')">
										Previous Month</a>
								</li>
								<li><a href="#"
									   onclick="load_income_custom('<?php echo date('Y'); ?>', '<?php echo date('Y'); ?>', 'current_y')">
										Current Year</a>
								</li>
								<li><a href="#"
									   onclick="load_income_custom('<?php echo date("Y", strtotime("-1 year")); ?>', '<?php echo date("Y", strtotime("-1 year")); ?>', 'previous_y')">
										Previous
										Year</a>
								</li>
								<li><a href="#"
									   onclick="load_income_custom('<?php echo 'ALL'; ?>', '<?php echo 'ALL'; ?>', '<?php echo 'ALL'; ?>')">
										List All</a>
								</li>
								<li><a href="#" onclick="income_list_datepicker()">Custom range</a>
								</li>
							</ul>
						</div>
    ...
    <script>             
            ...
            function print_window(  )
            {
                ...
                var url = "<?php echo site_url("income/print_income_custom"); ?>";
    ...
    <script>
        ...
        function load_income_custom(from, to, type) {
	    ...
	    $("#record_listings").html('<div class="loader"></div>');   
            ...
            $.ajax({
                type: "post",
                url: "<?php echo site_url("income/get_income_custom")?>/",
        ...
        function load_income_custom_with_dates() {
            $('#ModaldatePicker').modal('hide');
            load_income_custom($("#date_from").val(), $("#date_to").val(), 'custom');
        }

        function income_list_datepicker() { // custom listoing datepicker loader
            ...
            $("#ModaldatePickerSubmitBtn").attr("onclick", "load_income_custom_with_dates()");
        ...
        function edit_income(id) {
            ...
            $.ajax({
                type: "post",
                url: "<?php echo site_url("config/ajax_load_form")?>/",
                data: {form_name: 'income/income_edit_form', id: id},
                success: function (html) {
                    $("#myModalLabel").html('Edit/Update Income');
                    $("#myModalBody").html(html);
                    //$('#myModal').modal('show');
                },
                error: function (error) {
        ...
        function delete_income(income_id) {
            ...
            if (r == true) {
                ...
                $.ajax({
                    type: "post",
                    url: "<?php echo site_url("income/delete_income")?>",
