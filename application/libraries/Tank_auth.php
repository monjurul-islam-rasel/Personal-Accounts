...
class Tank_auth
{
	...
	function create_user_by_admin($username, $email, $phone, $user_type, $full_name, $password, $email_activation)
	{
	...
	function update_password($action_user , $new_pass)
	{
		$user_id = $action_user;
	...
	function delete_user_by_admin($user_id_to_delete)
	{
		$user_id = $this->ci->session->userdata('user_id');

		if ($this->ci->session->userdata('user_id') != $user_id_to_delete) 
		{		
			$this->ci->users->delete_user($user_id_to_delete);
